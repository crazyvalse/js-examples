 ...
 *@author firebug
 *@author caoly(caoly@founder.com)
 *@date 20150301
 */
(function () {
  var Xpath = window.Xpath = {};
  //origin copy from firebug/lib/xpath.js
  // ********************************************************************************************* //
  // XPATH
   ...
   *需找相对路径
   */
  Xpath.getElementRelativeXPath = function (element, parentEle) {
    return Xpath.getElementTreeXPath(element, parentEle);
  };

  Xpath.getElementTreeXPath = function (element, parentEle) {
    var paths = [];
    var relative = (parentEle != undefined && parentEle != null);
    var foundParentEle = false;

    // Use nodeName (instead of localName) so namespace prefix is included (if any).
    for (; element; element = element.parentNode) {
      if (relative && element.isSameNode(parentEle)) {
        foundParentEle = true;
        break;
      }
      var index = 0;
      var hasFollowingSiblings = false;
      var hasId = (element.id !== "" && element.id !== undefined);
      var hasClass = (element.className !== "" && element.className !== undefined);
