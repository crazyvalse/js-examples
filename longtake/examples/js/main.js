...
function setup () {
  ...
  initTouch()
  initTimeline(parentSprite)
}

function initTimeline (sprite) {
  // 在30%位置开始编号
  let timeline = new TimelineMax({ delay: 0.3 })
  // let tween = new TweenMax(sprite, 0.1, { alpha: 1 })
  // timeline.add(tween, 0)

  timeline.fromTo(sprite, 0.1, {
    alpha: 0
  }, {
    alpha: 1
  })
  mainTimeline.add(timeline, 0)
}
...
function initTouch () {
  ...
  let alloyTouch = new AlloyTouch({
    ...
    change: function (value) {
      console.info(value / min)
      mainTimeline.seek(value / min)
    }
  })
}
